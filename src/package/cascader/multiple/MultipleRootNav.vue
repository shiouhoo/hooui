<template>
    ...
    <RootNav
        ...
        :update-option-map="props.updateOptionMap"
...
const props = withDefaults(defineProps<{
    ...
    updateOptionMap: (value: Record<string, any>[]) => void;
}>(), {
    index: 0,
    lazy: false,
    isFinished: false,
...
function updateSelect(value: ((string | number)[])[]) {
    ...
    for(const data of props.treeData) {
        const currentValue = value.map((item)=>item.slice(-1)[0]);
        if(currentValue.includes(data.value)) {
...
function updateNextOptions(record: Record<string, any>) {
    ...
    unwatchMap[record.value] = watch(()=>record.children?.length, (length: number)=>{
        ...
        if(length && record.value === expand.value) {
            nextTreeData.value = record.children || [];
            props.updateOptionMap(record.children);
        }
...
function updateOptions() {
    if(props.parentRecord?.value) {
        ...
        // 第一级没有父级，监听treeData的变化
        const unWatch = watch(()=>props.treeData.length, ()=>{
            props.updateOptionMap(props.treeData);
            unWatch();
        });
    }
}
